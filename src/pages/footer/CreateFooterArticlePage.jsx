...
const CreateFooterArticlePage = () => {
  ...
  const handleAddSelected = async () => {
    ...
    const article = articles.find(
      (a) => a._id === selectedId || a.id === selectedId
    );
    if (!article) return toast.error("Article not found");

    const title = article.title || article.heading || "Untitled";
    const href = article.slug
      ? `/artikler/${article.categoryId.slug}/${article.slug}`
      : article.href || `/artikler/${article._id}`;
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="Add Article to Footer"
        description="Select an article to add to the footer"
      />
